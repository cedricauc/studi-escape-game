...
from main.views.views import LoginView, RegisterView, LogoutView, ManageProfileView, HomeView, ScenarioView, FaqView, \
    BookingView, BookingSumView, BookingFinalView, ManageOrderView, ManageChatView
...
def test_manage_chat_url():
    """ Tester si la route 'chat' correspond à ManageChatView """

    url = reverse('chat')
    assert resolve(url).view_name == 'chat'
    assert resolve(url), ManageChatView
